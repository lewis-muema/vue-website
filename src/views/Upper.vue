  ...
  <div>
    <div class="section section__upper flex centerY">
      ...
      <div class="section__upper_container flex rightX absolute centerY" :style="{ width: newWidth + '%', right: newLeft + '%' }">
        ...
        <router-link
          v-if="(parentName == 'Home' || parentName == 'AllCities' || parentName == 'Nairobi' || parentName == 'Mombasa' || parentName == 'Kisumu' || parentName == 'Thika' || parentName == 'Merchant' || parentName == 'Enterprise') && windowWidth > '768'"
          to="/partners"
          class="link"
          @click="
            mixTrackElements('Drive With Sendy');
            send_ga_event('Drive with Sendy Header - landing page - website');
          "
        >
          BECOME A PARTNER</router-link
        >
        <router-link
          v-if="(parentName == 'AllCities' || parentName == 'Nairobi' || parentName == 'Mombasa' || parentName == 'Kisumu' || parentName == 'Thika') && windowWidth <= '768'"
          to="/partners"
          class="link"
          @click="
            mixTrackElements('Drive With Sendy');
        ...
        <router-link v-if="windowWidth > '768'" to="/login" class="link upper__link"> LOG IN</router-link>
        <router-link to="/signup" class="link-margin" v-if="parentName == 'API' || parentName == 'Home' || parentName == 'Partners' || parentName == 'Careers' || parentName == 'SeniorPortManager' || parentName == 'ClientRelationsManager' || parentName == 'AndroidEngineer' || parentName == 'DataAnalyst' || parentName == 'QualityAssuranceTester' || parentName == 'HeadOfFinance' || parentName == 'KisumuCityManager' || parentName == 'Terms' || parentName == 'Privacy' || parentName == 'DriverPrivacy' || parentName == 'HeadOfPartnerOperations' || parentName == 'JuniorOperationsAssociate' || parentName == 'Merchant' || parentName == 'Enterprise' || this.parentName == 'CPS' || this.parentName == 'GMUG' || this.parentName == 'JRsupport' || this.parentName == 'MarketingAssociateUG' || this.parentName == 'OperationsAssociateUG' || this.parentName == 'OperationsAssociate' || this.parentName == 'ProductManager'"
          ><div
            class="button button__link bg-blue bc-blue orange-white flex centerX centerY"
            type="button"
            @click="
              mixTrackElements('Sign up Modal');
              send_ga_event('Sign up Header - landing page - website');
            "
          >
            SIGN UP
          </div></router-link
      ...
      <div class="section dropdown-container absolute overflow" v-if="showTable == true">
        <table :class="dropName" v-if="showTable == true">
          <tr>
            <td class="dropdown-upper-rows" v-if="parentName == 'API'">HOME</td>
            <td v-if="parentName !== 'API'" class="dropdown-upper-rows"><router-link to="/partners" class="color-white">BECOME A PARTNER</router-link></td>
          </tr>
          <tr>
            <td class="dropdown-upper-rows"><router-link to="/login" class="color-white">LOG IN</router-link></td>
          </tr>
          <tr>
            <td class="dropdown-upper-rows">
              <router-link
                to="/signup"
                class="color-white"
                @click="
          ...
          <tr>
            <td class="dropdown-upper-rows"><router-link to="/e-commerce" class="color-white">E-COMMERCE</router-link></td>
          </tr>
          <tr>
            <td class="dropdown-upper-rows"><router-link to="/enterprise" class="color-white">ENTERPRISE</router-link></td>
          </tr>
        </table>
      </div>
    </div>
